...
- Dev builds sign with a Developer ID-backed "Apple Development" identity (stable across rebuilds — ad-hoc signing broke status-item scene registration between builds). Release builds use the "Developer ID Application" identity.
- **Both flavours must be signed with entitlements.** Under the hardened runtime, `com.apple.security.device.audio-input`/`.camera` are what actually let the process open the mic and camera — a TCC grant alone is not enough, so an entitlement-less build looks "allowed" in System Settings while every recording silently fails. Release builds must still not carry `get-task-allow` (Apple's notary service rejects it), so the script copies `Resources/NotchActivityBar.entitlements` and strips that one key rather than keeping a second file that can drift. It then re-reads the signed bundle's entitlements and fails the build if the audio/camera keys didn't survive.
...
- **Permissions** — `PermissionsController` is the single source of truth for the three TCC grants recording needs (Microphone, Speech Recognition, Screen Recording). It re-reads them on app activation, on wake, when the Settings tab appears, and before each recording — macOS never notifies an app about a TCC change, so anything sampled once goes stale and reads as a mysterious silent failure. Screen Recording is special: CoreGraphics can't distinguish "never asked" from "refused", and a grant made after launch doesn't reach the running process, so the controller tracks the launch-time value and surfaces a Relaunch affordance.
